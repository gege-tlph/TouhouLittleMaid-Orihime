 ...
 * {@link #getCapacity(ItemVariant)} can be overridden to change the maximum capacity depending on the item resource.
 */
public abstract class ItemStackResourceHandler extends SnapshotJournal<ItemStack> implements ResourceHandler<ItemVariant>, ValueIOSerializable {
     ...
     * <ul>
     * <li>{@link #isValid(int, ItemVariant)}, to report which items are valid;</li>
     * <li>{@link #getCapacityAsLong(int, ItemVariant)}, to report a capacity of {@code 0} for invalid items;</li>
     * <li>{@link #insert(int, ItemVariant, int, TransactionContext)}, to reject items that cannot fit in this handler.</li>
     * </ul>
     */
    ...
    public long getCapacityAsLong(int index, ItemVariant resource) {
        Objects.checkIndex(index, size());
        return resource.isBlank() || isValid(resource) ? getCapacity(resource) : 0;
    }
