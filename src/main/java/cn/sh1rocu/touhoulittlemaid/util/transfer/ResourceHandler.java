...
public interface ResourceHandler<T extends TransferVariant<?>> extends SlottedStorage<T> {
    /**
     ...
     * @implSpec This method should return 0 for any resource for which {@link #isValid(int, TransferVariant)} returns {@code false}.
     * @see #getCapacityAsInt(int, TransferVariant)
     */
     ...
     * @implNote This method should not be implemented. The default method will call {@link #getCapacityAsLong(int, TransferVariant)} and convert the result appropriately.
     * @see #getCapacityAsLong(int, TransferVariant)
     */
    @ApiStatus.NonExtendable
     ...
     * @throws IllegalArgumentException If the resource is empty or the amount is negative. See also {@link StoragePreconditions#notBlankNotNegative} to help perform this check.
     ...
     * @see #insert(TransferVariant, int, TransactionContext) Inserting without a specific index, which can be more efficient.
     */
     ...
     *
     * <p>This function is preferred to the {@linkplain #insert(int, TransferVariant, int, TransactionContext) index-specific overload}
     ...
     * @implSpec Implementations must properly support {@linkplain Transaction transactions}.
     * Note that {@link SnapshotJournal} can serve as the base class for a transaction-aware resource handler.
     * @see #insert(int, TransferVariant, int, TransactionContext) Inserting into a specific index of the handler.
     */
     ...
     * @implSpec Implementations must properly support {@linkplain Transaction transactions}.
     * Note that {@link SnapshotJournal} can serve as the base class for a transaction-aware resource handler.
     * @see #extract(TransferVariant, int, TransactionContext) Extracting without a specific index, which can be more efficient.
     */
     ...
     *
     * <p>This function is preferred to the {@linkplain #extract(int, TransferVariant, int, TransactionContext) index-specific overload}
     ...
     * @implSpec Implementations must properly support {@linkplain Transaction transactions}.
     * Note that {@link SnapshotJournal} can serve as the base class for a transaction-aware resource handler.
     * @see #extract(int, TransferVariant, int, TransactionContext) Extracting from a specific index of the handler.
     */
    ...
    static <T extends TransferVariant<?>> Class<ResourceHandler<T>> asClass() {
    ...
    default SingleSlotStorage<T> getSlot(int slot) {
        return new SingleSlotStorage<>() {
            ...
            public long insert(T resource, long maxAmount, TransactionContext transaction) {
                return ResourceHandler.this.insert(slot, resource, (int) maxAmount, transaction);
            }

            @Override
            public long extract(T resource, long maxAmount, TransactionContext transaction) {
                return ResourceHandler.this.extract(slot, resource, (int) maxAmount, transaction);
            }

            @Override
            public boolean isResourceBlank() {
                return ResourceHandler.this.getResource(slot).isBlank();
            }

            @Override
            public T getResource() {
                return ResourceHandler.this.getResource(slot);
            }

            @Override
            public long getAmount() {
                return ResourceHandler.this.getAmountAsLong(slot);
            }

            @Override
            public long getCapacity() {
                return ResourceHandler.this.getCapacityAsLong(slot, getResource());
            }
        };
    }
    ...
    default long insert(T resource, long maxAmount, TransactionContext transaction) {
        return insert(resource, (int) maxAmount, transaction);
    }

    @Override
    default long extract(T resource, long maxAmount, TransactionContext transaction) {
        return extract(resource, (int) maxAmount, transaction);
    }
