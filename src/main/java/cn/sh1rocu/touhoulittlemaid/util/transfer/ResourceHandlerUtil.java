...
public final class ResourceHandlerUtil {
    ...
    /**
     * Determines if either the given resource or amount is classified as empty: if either {@link TransferVariant#isBlank()} ()} is {@code true},
     * or the amount is zero (or negative) then the resource is considered empty.
     *
     * @param resource The resource to check.
     * @param amount   An amount to check.
     * @return {@code true} if either {@link TransferVariant#isBlank()} returns {@code true}, or the amount is {@code <= 0}.
     */
    public static <T extends TransferVariant<?>> boolean isBlank(T resource, int amount) {
     ...
     */
    public static <T extends TransferVariant<?>> boolean isBlank(ResourceHandler<T> handler) {
     ...
     * greater than or equal to their respective {@linkplain ResourceHandler#getCapacityAsLong(int, TransferVariant) capacity}.
     ...
     */
    public static <T extends TransferVariant<?>> boolean isFull(ResourceHandler<T> handler) {
        int size = handler.size();
        for (int i = 0; i < size; i++) {
     ...
     * @see ResourceHandler#isValid(int, TransferVariant)
     */
    public static <T extends TransferVariant<?>> boolean isValid(ResourceHandler<T> handler, T resource) {
     ...
     */
    public static <T extends TransferVariant<?>> int getRedstoneSignalFromResourceHandler(ResourceHandler<T> handler) {
     ...
     *                    Passing in {@code null} will openNested a root transaction, and commit it at the end of the method.
     * @return the amount of the resource that was inserted
     */
    public static <T extends TransferVariant<?>> int insertStacking(
        ...
        try (Transaction tx = Transaction.openNested(transaction)) {
            ...
            for (int index = 0; index < size; index++) {
                if (!handler.getResource(index).isBlank()) {
    ...
    // Fabric
    public static <T extends TransferVariant<?>> long insertStacking(
            @Nullable SlottedStorage<T> handler,
            T resource,
            int amount,
            @Nullable TransactionContext transaction) {
        if (handler == null || amount == 0) return 0;
        ...
        try (Transaction tx = Transaction.openNested(transaction)) {
            long inserted = StorageUtil.insertStacking(handler.getSlots(), resource, amount, tx);
            tx.commit();
            return inserted;
        }
    }
     ...
     *                    Passing in {@code null} will openNested a root transaction, whereas passing in a transaction will
     ...
     */
    @Nullable
    public static <T extends TransferVariant<?>> ResourceAmount<T> extractFirst(
            @Nullable ResourceHandler<T> handler,
            Predicate<T> filter,
            int amount,
            @Nullable TransactionContext transaction) {
     ...
     */
    public static <T extends TransferVariant<?>> int move(
     ...
     */
    public static <T extends TransferVariant<?>> int moveStacking(
            @Nullable ResourceHandler<T> from,
            @Nullable ResourceHandler<T> to,
            Predicate<T> filter,
            int amount,
    ...
    private static <T extends TransferVariant<?>> int moveInternal(
            @Nullable ResourceHandler<T> from,
            @Nullable ResourceHandler<T> to,
            Predicate<T> filter,
            int amount,
        ...
        try (Transaction subTransaction = Transaction.openNested(transaction)) {
            int totalMoved = 0;
            int size = from.size();

            for (int index = 0; index < size; ++index) {
                T fromResource = from.getResource(index);
                if (fromResource.isBlank() || !filter.test(fromResource)) continue;

                // check how much can be extracted
                int maxExtracted;
                try (Transaction simulatedExtract = Transaction.openNested(subTransaction)) {
                    maxExtracted = from.extract(index, fromResource, amount - totalMoved, simulatedExtract);
                }

                if (maxExtracted == 0) continue;

                try (Transaction transferTransaction = Transaction.openNested(subTransaction)) {
     ...
     */
    @Nullable
    public static <T extends TransferVariant<?>> ResourceAmount<T> moveFirst(
            @Nullable ResourceHandler<T> from,
            @Nullable ResourceHandler<T> to,
            Predicate<T> filter,
            int amount,
     ...
     */
    @Nullable
    public static <T extends TransferVariant<?>> ResourceAmount<T> moveFirstStacking(
            @Nullable ResourceHandler<T> from,
            @Nullable ResourceHandler<T> to,
            Predicate<T> filter,
            int amount,
    ...
    @Nullable
    private static <T extends TransferVariant<?>> ResourceAmount<T> moveFirstInternal(
            @Nullable ResourceHandler<T> from,
            @Nullable ResourceHandler<T> to,
            Predicate<T> filter,
            int amount,
        ...
        try {
            ...
            for (int index = 0; index < size; ++index) {
                ...
                if (selectedResource == null && (fromResource.isBlank() || !filter.test(fromResource))
                ...
                // check how much can be extracted
                int extracted;
                try (Transaction simulatedExtractTransaction = Transaction.openNested(transaction)) {
                    extracted = from.extract(index, fromResource, amount - totalMoved, simulatedExtractTransaction);
                }

                if (extracted == 0) continue;

                try (Transaction transferTransaction = Transaction.openNested(transaction)) {
                    // check how much can be inserted
                    int inserted;
                    if (stacking) {
            ...
            return totalMoved > 0 ? new ResourceAmount<>(selectedResource, totalMoved) : null;
        } catch (Exception e) {
     ...
     */
    public static <T extends TransferVariant<?>> boolean contains(ResourceHandler<T> handler, T resource) {
     ...
     */
    public static <T extends TransferVariant<?>> int indexOf(ResourceHandler<T> handler, T resource) {
        StoragePreconditions.notBlank(resource);
        int size = handler.size();
        for (int index = 0; index < size; index++) {
     ...
     *                    {@code null} can be passed to conveniently have this method openNested its own root transaction.
     * @param <T>         The type of resource handled by the handler.
     * @return The first non-empty resource that matches the filter and is extractable, or {@code null} otherwise.
     */
    @Nullable
    public static <T extends TransferVariant<?>> T findExtractableResource(
            ResourceHandler<T> handler,
            Predicate<T> filter,
            @Nullable TransactionContext transaction) {
        try (Transaction temp = Transaction.openNested(transaction)) {
            int size = handler.size();
            for (int index = 0; index < size; index++) {
                T resource = handler.getResource(index);
                if (!resource.isBlank() && filter.test(resource) && handler.extract(index, resource, handler.getAmountAsInt(index), temp) > 0) {
                    return resource;
                }
            }
            return null;
