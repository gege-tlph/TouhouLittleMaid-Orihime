...
public final class ChatTokensCommand {
    ...
    private static int handleTokens(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        for (Player player : players) {
            switch (type) {
                case "set":
                    player.getAttachedOrCreate(InitDataAttachment.CHAT_TOKENS).set(count);
                    break;
                case "add":
                    player.getAttachedOrCreate(InitDataAttachment.CHAT_TOKENS).add(count);
                    break;
                case "min":
                    player.getAttachedOrCreate(InitDataAttachment.CHAT_TOKENS).min(count);
                    break;
                default:
            }
        }
    ...
    private static int getTokens(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        for (Player player : EntityArgument.getPlayers(context, TARGETS_NAME)) {
            ChatTokensAttachment tokens = player.getAttachedOrCreate(InitDataAttachment.CHAT_TOKENS);
