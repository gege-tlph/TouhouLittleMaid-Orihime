...
public final class GameContextRegister {
    ...
    /**
     * 注册一个上下文分类。 <p> 分类应尽量保持语义稳定，便于模型通过 tool 只加载当前所需的一类上下文， 从而减少 token 占用并提升上下文选择的可预测性。
     *
     ...
     * 适用于对话中经常需要但又不太占 token 的上下文。<br> 注意：如果一个分类被标记为 prompt context，那么它将不存在于 query_game_context 工具中
     */
    ...
    /**
     * 将一个上下文项注册到已存在的分类中。 <p> 调用前必须先通过 {@link #registerCategory(String, String, boolean)} 注册分类，或者使用已经注册过的分类 否则会抛出异常。没有上下文项的分类不会出现在 maid_context skill 提供给模型的可选分类列表中。
     *
