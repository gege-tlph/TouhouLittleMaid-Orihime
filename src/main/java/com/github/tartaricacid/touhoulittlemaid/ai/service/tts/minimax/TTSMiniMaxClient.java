...
public class TTSMiniMaxClient implements TTSClient {
    ...
    @Override
    public void handleResponse(TTSResponse callback, HttpResponse<byte[]> response,
                               @Nullable Throwable throwable, HttpRequest request) {
        // 结果是否已经没有意义，问回调自己——女仆链路上 isObsolete() 的默认实现就是
        // 「女仆没了或死了」，与旧的 shouldStopChat(maid) 判据逐字相同，行为不变；
        // 而试听没有女仆，只有问回调才不会被误判成「该丢弃」。
