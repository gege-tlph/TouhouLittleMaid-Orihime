...
/**
 * 方块食物接口，用于让女仆与方块类食物进行交互。 <p> 女仆会在工作日程时寻找并食用周围实现了此接口的方块食物。 此外，女仆还可以将背包中的食物物品放置为方块食物后再进行食用。 <p> 实现此接口的方块需要处理以下两种场景： <ul> <li>世界中已存在的方块食物的食用逻辑</li> <li>从物品堆放置为方块食物的逻辑（可选）</li> </ul>
 */
...
public interface IMaidEdibleBlock {
    ...
    /**
     * 判断女仆是否应该移动到该方块食物处进行食用。 <p> <b>性能提示：</b>此方法会被频繁调用，请确保实现的高效性。
     *
     * @param maid 女仆实体
     * @param pos 方块位置
     * @param state 方块状态
    ...
    /**
     * 执行女仆食用方块食物的逻辑。 <p> 实现此方法时，你需要自行处理方块状态的变更（如减少食物层数或移除方块）。
     *
     * @param maid 女仆实体
     * @param pos 方块位置
     * @param state 方块状态
    ...
    /**
     * 判断该物品是否可以被女仆放置为方块食物。 <p> 此方法用于过滤女仆背包中可放置的食物物品。
     *
     * @param maid 女仆实体
     * @param stack 待检测的物品堆
    ...
    /**
     * 判断女仆是否应该将物品堆放置到指定位置作为方块食物。 <p> <b>性能提示：</b>此方法会被频繁调用，请确保实现的高效性。
     *
     * @param maid 女仆实体
     * @param pos 待放置的位置
    ...
    /**
     * 执行女仆将物品放置为方块食物的逻辑。 <p> 实现此方法时，你需要自行处理物品数量的减少以及方块的放置逻辑。
     *
     * @param maid 女仆实体
     * @param pos 待放置的位置
     * @param stack 待放置的物品堆
