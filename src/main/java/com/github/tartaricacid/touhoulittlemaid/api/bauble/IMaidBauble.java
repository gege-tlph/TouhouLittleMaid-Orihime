...
public interface IMaidBauble {
    ...
    /**
     * 当女仆好感度等级发生改变时，调用此方法 <p> 虽然女仆设定上是只能升级不会降级，但是仍然有创造模式道具可以强制降级。<br> 所以也需要考虑上 oldLevel 和 newLevel 的大小
     *
     * @param maid 女仆
     * @param baubleItem 饰品物品
     * @param oldLevel 旧等级
    ...
    /**
     * 是否将当前佩戴的饰品同步到客户端。 <p> 默认情况下不进行同步。若饰品需要在客户端渲染特殊佩戴效果（例如自定义模型）， 应返回 true，服务器会把饰品物品数据发送到客户端以供渲染使用。
     *
     * @param maid 女仆实体
