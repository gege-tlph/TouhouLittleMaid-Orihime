 ...
 * <p>
 * <b>每一条语义都照抄 {@code port/1.21.11-fabric} 分支同名类的实现</b>
 * （那条分支移植自 {@code origin/1.21.1} 的挂件 layer：
 ...
 *       <p>⚠️ 这与 {@code GeoModelState#visitLocatorGroup} 的「{@code mulPose(累积矩阵)} +
 *       {@code translate(pivot/16)}」<b>不是同一个公式</b>，也没有人证明过两者等价——它们是两套模型系统
 *       各自的约定。本类的正确性判据不是「与 gecko 那条路等价」，而是<b>「与 {@code port/1.21.11-fabric}
 *       在 ILocationModel 这条路上的行为逐字一致」</b>。改动本类时请回去比对那条分支，不要拿 gecko 那条路当参照。</p></li>
 *   <li><b>手部 = 主链 + 每条 extra 链</b>：先按主链画一次，再对 {@code extraLeftHandBones()} /
 *       {@code extraRightHandBones()} 里的每条链各画一次。所以一个定位组可以对应多个定位点，
 *       与 TLM gecko 那侧「一个定位组可含多根骨骼」同构。</li>
 *   <li><b>缩放全为 0 则跳过</b>：用 {@code prepMatrixForLocator} 的返回值当门禁（模型作者靠把
 *       祖先骨骼缩放归零来隐藏挂点）。<b>注意两条路径的判据本就不同</b>：TLM gecko 那侧在
 *       {@code GeoModelStateExtractor} 里用的是「任意<b>两个</b>分量为 0」，这里是「<b>三个</b>分量全为 0」。
 *       不是笔误，各自照抄各自的基准。</li>
 * </ul>
 * <p>
 * ⚠️ {@link GeoLocatorType} 有意不是枚举（见其类注释），故这里用 {@code ==} 比较静态常量，
 * 不能写 {@code switch}。
 */
public final class LocationModelLocatorSource implements IGeoLocatorSource {
    ...
    /**
     * <b>这是"过滤前"的定位点数</b>，与 {@code !model.backpackBones().isEmpty()} 这类判据一致。
     * <p>
     * 唯一的消费者是 {@code GeckoLayerMaidBackItem}，它拿这个数只为回答"这个模型有没有背包定位组"，
