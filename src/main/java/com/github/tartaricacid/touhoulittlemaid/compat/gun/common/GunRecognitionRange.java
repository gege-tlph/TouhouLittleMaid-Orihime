 ...
 * 2026-08-14 用户裁决按枪种取值（1.21.11 分支定案，B3），本树随行为基准带上。</p>
 *
 * <p>刻意只吃**小写枪种字符串**而不吃 TaCZ 的类型：这样映射本身可以在没有 TaCZ 的环境里被单元测试
 * 覆盖（TaCZ 是 compileOnly，运行期不在 classpath 上）。取字符串那一步留在
...
public final class GunRecognitionRange {
     ...
     * 返回的是配置对象而非值——三键是世界规则，调用方必须经 {@code ServerRuleConfig.get(...)} 解析
     */
    @Nullable
