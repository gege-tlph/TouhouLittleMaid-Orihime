...
/**
 * 与 1.21.11 分支的差异仅有类型收敛：宿主 {@code GeckoMaidEntity.getMaid()} 返回
 * {@code EntityMaid}（非 IMaid），{@code asEntity()} 一跳随之消失；行为与基准一致。
 */
...
public class GunGeckoAnimation {
    ...
    public static PlayState playGunHoldAnimation(AnimationEvent<GeckoMaidEntity<?>> event, ItemStack heldItem) {
        ...
        EntityMaid entity = event.getAnimatableEntity().getMaid();
        if (entity == null) {
            return PlayState.STOP;
        }
        if (isMaidCarrying(entity)) {
            return PlayState.STOP;
        }
        ...
        if (reloadProgress > 0) {
            if (reloadProgress == 1) {
                ...
                // 与 1.21.11 分支同款适配（宿主 geckolib3 同一 API 家族，实查 CodedAnimationController 仍有该方法）。
    ...
    private static PlayState getGunTypeAnimation(AnimationEvent<GeckoMaidEntity<?>> event, String weaponType, String prefix) {
        ...
        // 本树的 ConditionManager 是随 GeckoContainer 走的每模型实例，字段恒非 null，
    ...
    private static boolean isMaidCarrying(EntityMaid maid) {
