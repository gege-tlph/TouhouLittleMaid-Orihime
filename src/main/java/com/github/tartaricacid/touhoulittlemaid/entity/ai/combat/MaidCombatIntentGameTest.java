...
public class MaidCombatIntentGameTest {
    /** ConfigData attachment 的实体 NBT 落位（Fabric attachment v1 持久化格式）。 */
    private static final String ATTACHMENTS_TAG = "fabric:attachments";
    private static final String CONFIG_ATTACHMENT_TAG = "touhou_little_maid:config";
    private static final String RESPONSE_POLICY_KEY = "combat_response_policy";
    ...
    public void finalDamageSignalsRespectOffSelfAndProtectPolicies(GameTestHelper helper) {
        ...
        party.maid.getEmergencyCombatManager().setResponsePolicy(MaidCombatResponsePolicy.OFF);
        ...
        party.maid.getEmergencyCombatManager().setResponsePolicy(MaidCombatResponsePolicy.SELF_DEFENSE);
    ...
    public void localityStickinessAndUnreachableTimeoutBoundTheTarget(GameTestHelper helper) {
        ...
        assertTrue(helper, party.maid.getEmergencyCombatManager().beginEmergency(
                first, MaidTargetingContext.PROTECT_OWNER), "local protection target was rejected");
        assertFalse(helper, party.maid.getEmergencyCombatManager().beginEmergency(
        ...
        party.maid.getEmergencyCombatManager().tick(helper.getLevel());
        assertFalse(helper, party.maid.isEmergencyCombatActive(),
    ...
    public void responsePolicyPersistsAndOldOrInvalidValuesDefaultToProtectOwner(GameTestHelper helper) {
        ...
        source.getEmergencyCombatManager().setResponsePolicy(MaidCombatResponsePolicy.SELF_DEFENSE);

        CompoundTag saved = saveMaid(helper, source);
        assertTrue(helper,
                "self_defense".equals(configAttachment(saved).getStringOr(RESPONSE_POLICY_KEY, "")),
                "response policy was not saved using its stable serialized name");
        assertTrue(helper,
                loadMaid(helper, saved).getEmergencyCombatManager().getResponsePolicy()
                        == MaidCombatResponsePolicy.SELF_DEFENSE,
                "saved response policy did not survive entity reload");

        // 旧档（键缺失）→ 护主；坏值 → 护主，且不得把整个 ConfigData 打回默认
        CompoundTag oldSave = saved.copy();
        CompoundTag oldConfig = configAttachment(oldSave);
        oldConfig.remove(RESPONSE_POLICY_KEY);
        putConfigAttachment(oldSave, oldConfig);
        assertTrue(helper,
                loadMaid(helper, oldSave).getEmergencyCombatManager().getResponsePolicy()
        ...
        CompoundTag invalidConfig = configAttachment(invalidSave);
        invalidConfig.putString(RESPONSE_POLICY_KEY, "future_invalid_value");
        putConfigAttachment(invalidSave, invalidConfig);
        EntityMaid invalidLoaded = loadMaid(helper, invalidSave);
        assertTrue(helper,
                invalidLoaded.getEmergencyCombatManager().getResponsePolicy()
                        == MaidCombatResponsePolicy.PROTECT_OWNER,
                "invalid saved response policy did not safely default to Protect Owner");
        assertTrue(helper,
                invalidLoaded.getAttachedOrCreate(InitDataAttachment.CONFIG).isPickup(),
                "a lenient policy decode failure discarded the rest of ConfigData");
        helper.succeed();
    }
    ...
    public void nbtRestoreDoesNotCreatePlayerCommandSuppression(GameTestHelper helper) {
        ...
        assertTrue(helper, restored.getEmergencyCombatManager().beginEmergency(
    ...
    public void subConfigPolicyCommandClearsEmergencyAndSyncsTrackedValue(GameTestHelper helper) {
        ...
        ConfigData settings = party.maid.getAttachedOrCreate(InitDataAttachment.CONFIG)
                .setCombatResponsePolicy(MaidCombatResponsePolicy.OFF);
        MaidSubConfigPackage.apply(party.maid, settings);

        assertTrue(helper,
                party.maid.getEmergencyCombatManager().getResponsePolicy() == MaidCombatResponsePolicy.OFF,
    ...
    private static CompoundTag configAttachment(CompoundTag saved) {
        return saved.getCompoundOrEmpty(ATTACHMENTS_TAG).getCompoundOrEmpty(CONFIG_ATTACHMENT_TAG);
    }

    private static void putConfigAttachment(CompoundTag saved, CompoundTag config) {
        CompoundTag attachments = saved.getCompoundOrEmpty(ATTACHMENTS_TAG);
        attachments.put(CONFIG_ATTACHMENT_TAG, config);
        saved.put(ATTACHMENTS_TAG, attachments);
    }
    ...
    private static void assertContext(GameTestHelper helper, EntityMaid maid,
        ...
        if (!maid.isEmergencyCombatActive()
                || maid.getEmergencyCombatManager().getTargetingContext() != expected) {
