...
public class MaidRangedEmergencyGameTest {
    ...
    /**
     * O11：坐着的女仆挨打后不许再平移走位——但照旧原地还手。
     *
     * <p>用户实机报「女仆坐下时用弓还会走位」。根因不在「没判坐姿」，而在
     * <b>走位这条路绕过了移动闸</b>：它不设 {@code WALK_TARGET}，直接写 MoveControl
     * （{@code getMoveControl().strafe(...)}），所以兄弟任务
     * {@code MaidEmergencyWalkToTarget} 用的 {@code canBrainMoving()} 管不到它。
     * ——⚠️ 账本原先写「两个移动任务都不看坐姿」，那句只对走位任务成立：
     * 走过去那条其实一直是让出 {@code WALK_TARGET} 的，实查纠正。</p>
     *
     * <p><b>对照组不能省</b>：站着必须返回 true。只断言「坐着不走位」时，
     * 任务因为任何别的原因起不来（拿错武器、没目标）也会绿——那正是本仓库反复栽的
     * 「零覆盖恒绿」。另外坐姿是<b>写进去再读出来</b>的 fixture，必须显式断言写成功了：
     * setter 带过滤器是常态，写入被静默丢弃会让「期望 false」的断言集体假绿。</p>
     */
    @GameTest(maxTicks = 100)
    public void sittingMaidDoesNotStrafeDuringEmergency(GameTestHelper helper) {
        ...
        // 走玩家指令那支会设重入抑制窗口，与 armedMaid 同理，故用 without-player-command
        ...
        assertFalse(helper, strafes(helper, sitting, sitTarget),
                "坐着的女仆不该再平移走位");
        helper.succeed();
    }
    ...
    private static boolean strafes(GameTestHelper helper, EntityMaid maid, Zombie target) {
        ...
        var strafing = new MaidAttackStrafingAnyItemTask(
                MaidEmergencyCombatManager::isHoldingUsableRangedWeapon,
                (float) MaidEmergencyCombatManager.LOCAL_PROTECTION_RANGE, 0.5f);
