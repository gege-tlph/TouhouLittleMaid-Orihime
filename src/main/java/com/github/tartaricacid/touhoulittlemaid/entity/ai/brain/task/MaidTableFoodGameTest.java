...
public class MaidTableFoodGameTest {
    ...
    public void stealYieldsItsTargetAfterHoldingTooLong(GameTestHelper helper) {
        ...
        assertTrue(helper, brain.hasMemoryValue(InitBrains.TARGET_POS),
        ...
        // 让持有到期
        brain.setMemory(InitBrains.MAID_EDIBLE_HOLD_EXPIRY, level.getGameTime());
        ...
        assertTrue(helper, !brain.hasMemoryValue(InitBrains.MAID_EDIBLE_HOLD_EXPIRY),
    ...
    private static void armHold(Brain<EntityMaid> brain, BlockPos target, long expiry) {
        brain.setMemory(InitBrains.MAID_EDIBLE_BLOCK_ACTION, MaidEdibleBlockAction.TRY_STEAL);
        brain.setMemory(InitBrains.TARGET_POS, new BlockPosTracker(target));
        brain.setMemory(MemoryModuleType.WALK_TARGET,
                new WalkTarget(new BlockPosTracker(target), 0.6f, 0));
        brain.setMemory(InitBrains.MAID_EDIBLE_HOLD_EXPIRY, expiry);
    }
