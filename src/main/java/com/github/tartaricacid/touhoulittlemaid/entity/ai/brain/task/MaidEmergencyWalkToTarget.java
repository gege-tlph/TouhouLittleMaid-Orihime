...
public final class MaidEmergencyWalkToTarget {
    ...
    private static boolean setTarget(EntityMaid maid, float speedModifier,
                                     ...
                                     LivingEntity target) {
        if (!maid.getEmergencyCombatManager().isEmergencyActive()
                || !maid.getEmergencyCombatManager().canRunCombatActions()) {
            return false;
        }

        EntityTracker tracker = new EntityTracker(target, true);
        lookTarget.set(tracker);
        boolean rangedStandoff = MaidEmergencyCombatManager.isHoldingUsableRangedWeapon(maid)
                && maid.canSee(target)
                && maid.distanceToSqr(target) <= MaidEmergencyCombatManager.LOCAL_PROTECTION_RANGE
                * MaidEmergencyCombatManager.LOCAL_PROTECTION_RANGE;
