...
/**
 * STT 全局配置标签页：总开关、来源列表、麦克风选择、识别距离、代理地址。
 *
...
public class AIChatSettingsSTTConfigScreen extends AIChatSettingsHubScreen {
    ...
    private void addSourceRow(STTApiType type, int x, int y, int width) {
        ...
        this.addRenderableOnly((Renderable) (graphics, mouseX, mouseY, partialTick) -> {
            ...
            graphics.drawString(this.font, name, x + 8, y + 5, nameColor, false);
            int statusX = x + rowWidth - 6 - this.font.width(status);
            graphics.drawString(this.font, status, statusX, y + 5, statusColor, false);
        });
    }
     ...
     * 后来短暂经停过一个「再列一遍全部来源」的中间列表页；那一层的内容与本屏四行完全重复，
    ...
    private void openSourceConfig(STTSite site) {
        ...
        this.minecraft.setScreen(new STTSiteEditorScreen(this, site, this::saveLocalSTTSite));
    }
    ...
    /**
     * 本页的保存要做**两件事**：落盘本机 AI 配置，以及提交四页共享的世界规则暂存。
     *
     * <p>后半句原先漏了，而绿字「已保存」照发——于是「在文字模型页翻个开关 → 切到本页 → 保存」
     * 会告诉玩家保存成功，实际上那条世界规则一个字也没上服务器，退出即丢。
     * 本页是四页里唯一有本机配置的，最容易被当成「与世界规则无关」，也就最容易漏。
     * 契约是 hub 写死的：<b>在任何一页点保存都提交攒着的全部改动</b>。</p>
     */
