...
public class CtrlBinding extends ContextBinding {
    ...
    private CtrlBinding() {
        ...
        register("idle", Priority.LOWEST, unused -> true);

        // 轮盘动画预测
        var("playing_extra_animation", unused -> null);
    ...
    private void addModPlaceholder() {
        livingEntityVar("carryon_type", unused -> StringUtils.EMPTY);
        livingEntityVar("carryon_is_princess", unused -> false);
        livingEntityVar("swem_is_ride", unused -> false);
        livingEntityVar("swem_state", unused -> StringUtils.EMPTY);
        livingEntityVar("parcool_state", unused -> StringUtils.EMPTY);
        playerVar("create_hanging_skyhook", unused -> false);
        clientPlayerVar("bcombat_attack_animation", unused -> StringUtils.EMPTY);
        clientPlayerVar("iss_animation", unused -> "");
    }
