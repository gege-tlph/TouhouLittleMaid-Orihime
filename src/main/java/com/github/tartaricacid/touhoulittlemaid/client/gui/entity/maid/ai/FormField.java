...
public class FormField {
     ...
     *
     * <p>⚠️ 行为基准里这里还有一个 {@code cleared} 标志与 {@code clearSecret()} /
     * {@code cleared()} / {@code secretAlreadySet()} 三个方法，但**全仓零调用者**
     * （只有 LLM 编辑屏有「清除」按钮，而它跟踪的是自己的字段）：{@code cleared} 恒为假，
     * 依赖它的两处分支不可达。按「不留零消费者空壳」未搬，行为不变。
     * 将来给 TTS/STT 编辑屏加「清除」时，连同调用点一起加回来。</p>
     */
     ...
     *
     * <ul>
     *   <li>已配置且没动过（框是空的） → 哨兵：服务端把原值填回去，这是不丢密钥的关键</li>
     *   <li>填了新值 → 新值</li>
     * </ul>
     */
    public String value() {
        String current = this.box != null ? this.box.getValue() : this.value;
        if (this.secret && this.secretAlreadySet && current.isEmpty()) {
