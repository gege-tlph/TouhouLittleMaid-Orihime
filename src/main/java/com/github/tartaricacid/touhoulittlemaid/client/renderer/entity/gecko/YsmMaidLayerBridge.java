 ...
 *
 * <p><b>为什么入口在 TLM 而不是让接管方自己拼</b>：怎么构造 {@link GeckoMaidRenderData}、
 * 定位来源该塞什么、camera 传什么才安全，都是 TLM 的内部知识。放在这里，接管方只需要在
 * <b>模型空间</b>里调一次，日后 TLM 增删 layer、换数据载体都不必惊动对面。</p>
 *
 * <p><b>调用时机是硬要求</b>：必须在接管方自己的 {@code pushPose()}/{@code popPose()} 之内、
 * 模型的根变换（朝向、缩放、睡姿位移等）都施加完之后调用。挂件靠骨骼链定位，
 * 而骨骼链是相对模型根的；在实体根坐标系里调，挂件会整体错位。</p>
 *
 * <p>camera 取自 {@link EntityMaidRenderState#camera}，由 {@code EntityMaidRenderer.submit} 在
 * 移交给接管方<b>之前</b>填好，接管方自己也在用它（{@code RenderContext.enter(collector, state.camera)}）。
 * 所以这条路上 camera 是真的，不必像早先设想的那样传 null——目前 5 个 layer 都没读它，
 * 但将来谁要用，直接用即可，不需要改接口。</p>
 */
...
public final class YsmMaidLayerBridge {
    ...
    public static void submitMaidLayers(List<GeoLayerRenderer<?, ?>> layers,
        ...
        try (GeckoMaidRenderData data = new GeckoMaidRenderData()) {
            data.locatorSource = new LocationModelLocatorSource(locationModel);
