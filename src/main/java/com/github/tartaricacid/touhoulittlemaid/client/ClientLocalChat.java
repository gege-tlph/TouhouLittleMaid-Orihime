...
public final class ClientLocalChat {
    ...
    public static void show(Component message) {
        ...
        // 26.1.2 把 ChatComponent.addMessage 私有化，并按来源拆出了两个公开入口：
        // addServerSystemMessage（GuiMessageSource.SYSTEM_SERVER，原版 handleSystemMessage 走这条）
        // 与 addClientSystemMessage（SYSTEM_CLIENT）。**后者正是本方法要的语义**——
        // 这行字是本机自己打的提示，不是任何人发来的消息。两者只差 source 标记，渲染相同，
        // 都不记聊天日志（logSystemMessage 在 ChatListener 里，不在这条路上）。
