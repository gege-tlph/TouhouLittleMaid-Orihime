 ...
 * 各标签页（语音输入 / LLM / TTS / 用量）由子类实现 {@link #initContent()}。
 */
public abstract class AIChatSettingsHubScreen extends Screen {
    ...
    /** 「语音输入设置」的独立入口：侧栏只留那一栏，供聊天屏直接进本机语音设置 */
    ...
    /**
     * 行式布局的两个常量与三个助手：**所有设置屏的表单行都走这一套**，别再各排各的。
     *
    ...
    protected void addRowLabel(int y, Component text, @Nullable Component tooltip) {
        ...
        this.addRenderableOnly((Renderable) (graphics, mouseX, mouseY, partialTick) -> {
            graphics.text(this.font, text, x, y + 6, 0xFFC6C6C6, false);
            if (tooltip != null && mouseX >= x && mouseX < x + LABEL_WIDTH
                    && mouseY >= y && mouseY < y + 20) {
                graphics.setTooltipForNextFrame(this.font, List.of(tooltip), Optional.empty(), mouseX, mouseY);
            }
        });
    }
     ...
     *
     * <p>⚠️ 这条是**全称契约**，所以刀四配了一条按屏枚举的测试钉着它：
     * 我在别处写过三遍「任何一页保存都会提交全部改动」而语音输入页当时根本不碰共享暂存，
     * 写下的契约是意图不是证据。</p>
     */
     ...
     *
     * <p>站点是逐个保存的（各自走 {@code Save*SitePacket}），AI 规则不是——它们攒在
     * {@link ServerRulesClientCache.Session} 里一起提交，所以这两类东西虽然摆在同一屏，
     * <b>保存路径是两条</b>。屏上必须把这一点说清楚。</p>
     */
     ...
     * 世界默认（站点, 模型）对选择器的候选项。
     *
     * <p>候选全部来自**已启用**站点：没启用的站点选了也不会生效，列出来只会制造
     * 「设了没用」的困惑。</p>
     */
    protected record SitePair(String siteId, String modelId, String label) {
    }

    protected static <T extends Site> List<SitePair> sitePairs(Map<String, T> sites) {
        List<SitePair> pairs = new ArrayList<>();
        sites.forEach((id, site) -> {
            if (!site.enabled()) {
                return;
            }
            String siteName = siteDisplayName(id);
            if (site instanceof SupportModelSelect select && !select.models().isEmpty()) {
    ...
    protected static String pairLabel(List<SitePair> pairs, String siteId, String modelId, String builtinSiteId) {
        ...
        return siteDisplayName(StringUtils.isBlank(siteId) ? builtinSiteId : siteId);
    }

    protected static int pairIndex(List<SitePair> pairs, String siteId, String modelId) {
    ...
    private @Nullable List<HubPopupEntry> popupEntries;
    private int popupX;
    private int popupY;
    ...
    protected void openHubPopup(int x, int anchorY, int width, List<HubPopupEntry> entries) {
        if (entries.isEmpty()) {
            return;
        }
    ...
    /**
     * 覆盖层（弹出列表）绘制。**每个子类 extractRenderState 的最后一行必须调用它**——
     * 弹出层要压在滚动条、提示字这些子类自绘物之上，而那些都画在 super 之后。
     */
    protected void renderHubOverlays(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < this.popupVisible; i++) {
            ...
            graphics.text(this.font, label, this.popupX + 7, rowY + 3,
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        if (this.popupEntries != null) {
            ...
            if (inside && event.button() == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
    ...
    public boolean keyPressed(KeyEvent event) {
        if (this.popupEntries != null && event.key() == GLFW.GLFW_KEY_ESCAPE) {
            this.closeHubPopup();
            return true;
        }
     ...
     * 保存成功后的绿字：`*` 熄灭太含蓄。
     *
     * <p>没有专服变体：本枢纽的规则全是**实例级 AI 规则**，
     * {@code AiServerRuleConfig} 保存即激活、专服也一样，故不存在「重载后生效」那半句。</p>
     */
     ...
     */
    protected void renderSavedFlash(GuiGraphicsExtractor graphics) {
        if (this.savedFlashText == null || System.currentTimeMillis() > this.savedFlashUntil) {
            return;
        }
        graphics.text(this.font, this.savedFlashText,
    ...
    public static final class SharedState {
        ...
        /** 世界规则暂存，各标签页共享——切页不丢未保存改动，哪页点保存都提交全部 */
