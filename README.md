...
| Minecraft | 1.21.11 |
| Java | 21 |
| Fabric Loader | 0.18.0 或更高版本 |
| Fabric API | 0.140.0+1.21.11 或更高的 1.21.11 版本 |
| Forge Config API Port | 21.11.0+（**必装**） |
| 安装位置 | 客户端与服务端 |

以上下限是**实测出来的**，不是照抄构建时用的版本：Fabric API 逐版本编译到 0.140.0 仍然通过、
0.139.5 失败；Forge Config API Port 该版本线的全部版本都通过；Fabric Loader 在 0.18.0 上
跑完整 GameTest（76 项，0 失败）仍然全过，而 0.18.0 已是该系列最早的版本。
本模组自身不声明版本下限，第三方模组各有各的要求，请看它们自己的页面。
