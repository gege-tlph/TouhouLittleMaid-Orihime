...
You can also follow the [CurseForge project page](https://www.curseforge.com/minecraft/mc-mods/touhou-little-maid-tsumugi).
The project is currently under review, and the 26.1.2 files will appear there after approval.
...
## New gameplay in this branch

### Threat response

While idle, a maid can defend herself or protect her owner according to her configuration. Combat response is
temporary and does not replace the maid's scheduled work:

| Mode | Behavior |
|---|---|
| **Off** | Keep the original behavior |
| **Self-defense** | Fight back when the maid is attacked |
| **Protect owner** | Join a fight when the owner is attacked |

When holding a bow, crossbow or TACZ gun with ammunition, a maid keeps her distance and shoots instead of
charging directly into melee.

### Configuration and AI screens

The global configuration and AI settings screens have been reorganized. Server rules are synchronized to clients,
site settings can be checked in place, TTS voices can be previewed, and updated settings can be reloaded with
dedicated commands.

AI chat and voice still require players to provide their own service endpoints and credentials. This mod does not
provide or proxy third-party AI services.

## Fixes in this branch

- Fixed furnace backpack item ordering, fuel handling and smelting progress.
- Fixed off-hand items being removed when a maid eats.
- Fixed maids intercepting Loyalty tridents thrown by players.
- Fixed extinguishers failing against some soul-fire blocks.
- Fixed a client crash when opening a maid screen as the maid disappears.
- Fixed incomplete Patchouli manual contents, recipe references and missing-mod prompts.
- Fixed regressions affecting cushions, item previews, model markers, backpack screens and gun rendering.
- Fixed several cross-version resource-pack and client resource-reload issues.

## Compatibility not yet verified

The following mods do not currently have a fully verified Minecraft 26.1.2 Fabric integration in this Beta:

| Mod | Status |
|---|---|
| KubeJS | No verified 26.1.2 Fabric integration path yet |
| Aquaculture | No usable 26.1.2 Fabric build currently available |
| Accessories | No verified 26.1.2 Fabric integration path yet |
| Sophisticated Backpacks | Its slot compatibility path has not been verified |
| YSM / OpenYSM | No verified 26.1.2 Fabric maid compatibility module yet |

This is not a permanent exclusion. Compatibility will be reevaluated when a usable target build or a reliable
Fabric integration path becomes available.
...
Please use the matching issue template whenever possible. Beta testers are especially encouraged to report
problems found with clients, dedicated servers and third-party mod combinations.
