...
1. §3.J 测试与门禁设施 → 2. §3.A 配置三层 → 3. §3.B 威胁响应 → 4. §3.H 命令 →
   ...
   **原先「全部待核」的表是过期的，照它干活会重复移植** →
   ...
   9. §3.G 兼容。
   **下一项实为 §3.G**，且它**不再整体被前置卡住**：§2.1 实查 Kaleidoscope Tavern
   Refabricated（`1.2.0.5-fabric+mc26.1.2`）与 MrCrayfish's Furniture: Refurbished
   （`v1.0.23+26.1.2`）**都有 26.1.2 release**，两项「我们独有」的兼容可以开工；
   只有 YSM 那条仍等 O4。⚠️ 家具重制那条附带的注册表 SYNCED 改动要**重新判断代价**（见 §3.G）。
