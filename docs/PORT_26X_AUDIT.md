...
**2026-08-17 机械抽取到一条具体输入**（Brain 活动→行为→优先级 的四树差集对账，
方法见 §7.9 第四面）：**跟随主人与骑乘跟随的优先级，行为基准把它从 3 降到 4**
（并抽成具名常量），本树仍是 3、与宿主相同——这就是「主人跟随降级为兜底任务」的**全部改动**。
⚠️ 它的形态值得记住：**同文件、同类、同方法，只有一个数字不同**，
文件面 / 提交面 / 公开面三种枚举**全都照不出它**。这一层剩下的条目多半也是这个形态，
所以 §3.E 动手前先跑一遍那支指纹工具，把「要搬什么」变成一张机械产出的清单，
而不是从交接文档抄。
...
配置面是第一个。**其余子系统的行为面尚未对账**，按玩家可见程度排：
网络包（发不发、发给谁）→ 存档与 attachment（持久化什么、同步给谁）→
资源与注册（生成什么文件、注册进哪张表）→ GUI（哪个屏在什么权限下出现）。
⚠️ 别把这条读成「其余子系统有问题」——是**没查过**，与「查过没问题」不是一回事。
