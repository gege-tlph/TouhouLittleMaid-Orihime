...
1. §3.J 测试与门禁设施 → 2. §3.A 配置三层 → 3. §3.B 威胁响应 → 4. §3.H 命令 →
   ...
   **原先「全部待核」的表是过期的，照它干活会重复移植** →
   7. ~~§3.E 寻路手感~~ **已整块落地（2026-08-17）** → 8. §3.F 渲染 →
   9. §3.G 兼容（YSM / Patchouli 前置就绪后）。
   **下一项实为 §3.F**——⚠️ 它三项都依赖 1.21.11 的 RenderState / SubmitNodeCollector 形态，
   26.x 的渲染管线要重新读；YSM 那条还依赖 fork 侧，双侧同时才有效（见 §3.F 与 §7.2）。
