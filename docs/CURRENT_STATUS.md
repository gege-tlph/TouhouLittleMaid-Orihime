...
| **前置条件** | 需装 **Create Fly** `26.1.2-6.0.9-4`（Modrinth `create-fly`，mod id 是 `create`）到 `run/mods/`；jar 不入库 | dev 客户端单人档即可；备一把**带快速射击附魔**的弩与一把白板弩作对照 |
| **操作步骤** | ① 造一台机械手 ② 按祭坛配方摆料 ③ 看是否开始合成 | ① 女仆设非远程任务、手持上箭的弩 ② 挨打触发应战 ③ 看她与目标的进退距离 ④ 换附魔弩再来一次，比射速 |
| **预期结果** | 机械手**不合成**祭坛配方 | 弩在约 **1.6 格**才后退、约 **4 格**就压上（改前是 3.2 / 8）；附魔弩明显比白板弩射得快（改前一样快） |
| **失败判据** | 照样合成 = 标签没生效，查 `data/create/tags/recipe_serializer/automation_ignore.json` 是否随 jar 打包 | 进退距离仍是 3.2 / 8 = `getDefaultProjectileRange` 那支没走到（多半落进了回落分支）；附魔无差 = 附魔读点没生效 |
