...
**图标缓存刀实机项（2026-08-15 首轮实测后更新）**：

- ✅ **首轮实测（2026-08-15 用户截图）**：缓存屏弹出、跑完回原屏、图标全部生成且**无错位**
  ——第三次重推导的截图时序实机定案成立；活体渲染（左侧预览/世界内）均正常
- ⚠️ 首轮抓出**绿面伪影**：半透明贴图部件（琪露诺冰翅 α=128 实查）与绿幕混合后逃过
  origin 的等值抠像，图标整片变绿。**定性为 origin 固有伪影非移植回归**（origin 同款
  等值判定，机制数学必然）。用户批准超基准修复：**双背景差分抠像**（`7a2d63ca9`，
  逐像素反解 α 与前景色，纯函数五例 JUnit 钉住）
- ✅ 二轮实测：bedrock 模型绿面消失；⚠️ 抓出残留——**gecko 模型图标仍错**。定因：首版
  差分跨帧取两张（绿一帧、品红一帧），bedrock 近乎静止没暴露，gecko 骨骼动画的帧间位移
  被差分误读成透明度。改为**同帧双幕**（左绿右品红同帧各渲一次，同动画时间戳姿态逐位相同，
  时间变量整个消掉；捕获推迟到第 3 绘制帧给 gecko 动画绑定留缓冲）——**待三轮复验**：
  gecko 模型图标应正常；bedrock 照旧无绿面无错位；真绿部件不缺失
