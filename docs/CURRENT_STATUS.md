...
**O2 · 反向缺口：宿主迁移时丢掉的东西（新开，规模已量出）**

`PORT_26X_AUDIT` §3～§7 只回答「我们多出来的部分搬齐了没」，
**回答不了「宿主把 `origin/1.21.1` 原本就有的东西丢了多少」**——那类缺口不在任何差异化清单里。
工具 `docs/tools/host_gap.py`（可重跑），方法与首轮结果记在审计 §7.8。

首轮：残余 **262** 个消失的类，其中 37 个属 §2.2 生态不允许，**225 个待逐条判定**。
⚠️ **225 不等于 225 处回归**，已抽样确认至少三种「不是回归」的形态（架构替换 / 内部重构 / 库合并）。

**已确认的真实丢失**（首轮抽样，非全集）：

| 丢了什么 | 玩家侧后果 | 状态 |
|---|---|---|
| **4 种女仆背包：工作台 / 末影箱 / 熔炉 / 液体** | 这四种背包**整个玩法没了**。`entity/backpack/` 在 `origin/1.21.1` 与行为基准上都是 9 个类型，宿主只剩 5 个 | **未修，最大的一处** |
| 扛女仆时玩家手臂抱姿 | 第三人称手臂僵着 | 已修 `79e7f7914` |
| 女仆喂主人喝东西的音效 | 喂药水/牛奶没声音 | 已修 `6dfb3357e` |

下一步建议：先把 225 个按模块判定完（`client` 51 / `geckolib3` 25 / `api` 18 是大头），
**再决定背包那四种要不要补**——它涉及物品、容器、GUI、数据与渲染五层，是一整块功能。
