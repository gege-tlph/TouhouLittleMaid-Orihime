...
| — | 零散 | 1 | ~~`GifTexture`~~（**已补回 `b70145a20`**，宿主 FIXME 搁置还原）；剩 `RenderFixer`（Carry On 渲染修正，Carry On 2.10.0 有 26.1.2 正式版可接回） | 实时待补名单跑 `--ledger`；此前表里写的「6」把已随其它刀补掉/改判的算在内，机械口径以工具为准 |
