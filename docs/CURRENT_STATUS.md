...
⚠️ **2026-08-19 第一轮实测未能开始**：装上 Create Fly 后创造背包一开就崩，
根因是我们自己漏搬的一条修复（见已关闭表当日第一条，`f455a8e4d`）。修复后已重开客户端，
下表两项仍待验。

| | ① 祭坛不可被机械动力自动化（`421ee6b70`） | ② 弓/弩手感还原后的复验（`94338b210`） |
|---|---|---|
| **测试目的** | 机械手等自动化装置是否**拒绝**跑祭坛配方 | 三处还原后，应战期弓/弩的手感是否已与上游一致 |
| **前置条件** | ✅ **2026-08-19 已备好**：`create-fly-26.1.2-6.0.9-4.jar` 已在 `run/mods/`（Modrinth 下载，sha1 `7a60216e…37adb` 核对一致；`fabric.mod.json` 实读 **mod id = `create`**、loaders 仅 fabric、零 JiJ、除 loader/MC/Java 外无硬依赖）。jar 不入库 | dev 客户端单人档即可；备一把**带快速射击附魔**的弩与一把白板弩作对照 |
| **操作步骤** | ① 造一台**机械手（Deployer）**——⚠️ **别用机械臂（Mechanical Arm）**，它不读这个判据（见下）② 按祭坛配方摆料 ③ 看是否开始合成 | ① 女仆设非远程任务、手持上箭的弩 ② 挨打触发应战 ③ 看她与目标的进退距离 ④ 换附魔弩再来一次，比射速 |
| **预期结果** | 机械手**不合成**祭坛配方 | 弩在约 **1.6 格**才后退、约 **4 格**就压上（改前是 3.2 / 8）；附魔弩明显比白板弩射得快（改前一样快） |
| **失败判据** | 照样合成 = 标签没生效，查 `data/create/tags/recipe_serializer/automation_ignore.json` 是否随 jar 打包。⚠️ **两端契约已 2026-08-19 用 javap -c 对上**（Create 侧 `TagKey.create(Registries.RECIPE_SERIALIZER, create:automation_ignore)` + `wrapAsHolder(...).is(TAG)`；我方 `InitRecipes` 注册的正是标签里那个 `touhou_little_maid:altar_recipe`），故**剩下的失败形态只有「标签没进 jar」与「用错了机器」两种** | 进退距离仍是 3.2 / 8 = `getDefaultProjectileRange` 那支没走到（多半落进了回落分支）；附魔无差 = 附魔读点没生效 |
| **日志采集** | `run/logs/latest.log` 搜 `automation_ignore` / `create` | 无专门日志，靠目视；必要时插 `[TLM-QA-*]` 桩打印 `maxAttackDistance` 与蓄力门槛 |
| **测试后更新哪一条账本** | 本表 + 已关闭表里 `create:automation_ignore` 那条的「实机未验」一句 | 本表 + O1 上方 2026-08-18 验收段 |

**① 的消费者名单（2026-08-19 扫 Create Fly jar 字节码，24 个 class 引用该判据）**：真正的机器是
**机械手 Deployer · 机械合成器 · 机械压床 · 搅拌器 · 动力锯 · 鼓风扇加工（熔炼/烟熏）· 工厂面板**，
其余是 JEI/REI/EIV 三家的配方展示。**机械臂（Mechanical Arm）不在名单里**——用它测等于没测，
这正是「判据选错对象时红测会连同错误一起确认」那条教训的同形陷阱。

⚠️ 两项的**代码面都已由契约测试钉住**（`automation_ignore` 四条 / 弓弩还原三条，各自逐个红测过），
但契约测试只证明「接线在、数据在」，证明不了「玩家看到 / 手上感觉到的是对的」——
**这正是它们仍为 DEFERRED 的原因**。尤其第 ② 项：手感按定义没有机械判据。
...
或进应战时按危险逃离同式解除坐姿。**要不要修请裁决**（⚠️ 我是从代码路径推的，未在基准实机复现；
两树代码面逐字相同这点确凿）。
