...
剩下的开放项是：**实机验收**（专服侧整体 + §3.C 那一大批 + 四项 DEFERRED：② 弓弩手感 · ③ 打雪仗后一半 · ④ 雪球击退开关 · ⑤ 菜单重构与平滑跟随）、
...
**仍 DEFERRED — ⑤ 配置菜单重构 + 平滑跟随搬家（`0ff5f8295` `eaa9bbbcb`，2026-08-19 用户要求）**

| | ⑤ 菜单分组 · AI 单一入口 · 平滑跟随默认开 |
|---|---|
| **测试目的** | 重构后的菜单结构对不对；AI 入口按钮通不通；平滑跟随的新默认与旧存档继承 |
| **前置条件** | dev 客户端；**另需一份升级前存过 `[experimental] SmoothFollow = false` 的旧存档**才能验继承那一维 |
| **操作步骤** | ① modmenu 进配置：看「个人设置」是不是一栏三组、四个旧平铺栏目已消失 ② 点「AI 与语音设置 → 打开」，在世界里与标题屏各点一次 ③ 「玩法设置 → 工作与活动范围」里找平滑跟随 ④ 新建存档看它默认是开 ⑤ 用上述旧存档进一次，看它是否仍为关 |
| **预期结果** | ① 三组：声音与显示 / 外观与性能 / 交互提示；无裸键名；无饰品栏那一项（Trinkets 未装） ② 世界里打开的是完整五页设置屏；标题屏打开的是语音输入那一页 ③ 平滑跟随在范围滑条那一组，不在实验性 ④ 新档默认开 ⑤ 旧档仍为关，且 `serverconfig/touhou_little_maid-server.toml` 里 `[maid] SmoothFollow = false` |
| **失败判据** | 界面出现 `config.touhou_little_maid.*` 裸键 = lang 漏了（契约测试只查 en_us）；按钮点了没反应 = 入口链路断；旧档变成开 = 跨键继承没跑或次序反了（日志搜 `Carried`） |
| **日志采集** | 继承那一维看日志里的 `Carried [experimental, SmoothFollow] over to [maid, SmoothFollow]` |
| **测试后更新哪一条账本** | 本表 + 跨版本 wiki 的 A10 / A7 / E4 |

⚠️ **平滑跟随默认开是有意分歧**：本树开箱即用的跟随行为**不再等于行为基准**，
要基准那套起跟距离/速度/传送阈值得手动关掉它（用户 2026-08-19 决定）。
⚠️ 死键扫描当轮清零（10 条真死键），但**那是一次性脚本，没固化成测试**——见 wiki A10。
