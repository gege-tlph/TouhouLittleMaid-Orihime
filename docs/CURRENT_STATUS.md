...
**AI 设置屏五页（O7，`f01ae9bce`…`fcbd03296`）：同样零实机**。GUI 是玩家可见层，
自动化门只到「编译 + 契约」，逐屏点检必须入世。清单：

| 待验 | 判据 |
|---|---|
| 侧栏权限边界 | 非 OP / 局域网客机开设置屏：侧栏**只剩「语音输入设置」一栏**，且落点就在那一页（不是高亮一个看不见的标签） |
| 跨页共享暂存 | 在 LLM 页翻个开关 → 切到语音输入页 → 点保存 → 重开：那条 LLM 规则应已生效（这条是本刀契约的主角，测试只钉源码接线，值有没有真上服务器要实机看） |
| 弹出列表 | 默认模型/默认音色/麦克风三处：点开列全部候选、当前项高亮、下方放不下时向上弹、滚轮归弹层（底下列表不跟着滚）、点外面只关弹层不误触 |
| 试听 | TTS 页与聊天屏 TTS 弹层的 ▶：出声；请求中其余入口置灰（灰而不是画成正常再吞点击）；失败在屏内出红字而不是聊天栏 |
| 密钥三态 | 只改 URL 保存 → 密钥不丢；点「清除」保存 → 真的清空；框空着时灰字区分「已配置 / 未配置」 |
| 检查配置 | 按钮变判词 + 悬停给完整原因；8 秒后还原。系统 TTS 站点上**不应出现**保存/检查两颗 |
| TTS 语种诚实标注 | 系统 TTS 下 🌐 按钮悬停出黄色警告；系统站点编辑屏有两行说明 |
| STT 全局单选 | 侧栏无「语音输入站点」标签；来源四行各显示配置状态；齿轮进表单，返回回本页（不是关掉整个 UI） |
| 跟随默认三态 | 底部概要区分 跟随默认 / 已覆盖 / 已回落，且显示站点**名字**不是 id；新女仆开屏点一次弹出项后，未动过的那一项仍应是「跟随默认」 |
