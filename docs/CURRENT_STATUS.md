...
**O2 · `CLAUDE.md` 里有几条继承来的前提已不成立（开工前先处理，第一条尤其）**

方法论与证伪表是从 1.21.11 分支原样搬来的，价值在经验，但**前提是版本相关的**。逐条查过：

| 条目 | 结论 | 处理 |
|---|---|---|
| 核心纪律 **第 2 条** | **方向反了**。原文禁止「为贴近 26.1 改行为」——那是 1.21.11 语境（那时 `origin/26.1` 只是架构参照）。**本分支它是代码宿主** | **开工前就地改**：写法对新基 `origin/26.1`，行为对 `port/1.21.11-fabric` |
| 核心纪律 **第 8 条** | 「仓库约定 `tileentity/TileEntity*`」**是假的**——新基是 `blockentity/BlockEntity*`（祭坛叫 `BlockEntityAltar`）。但同条另两半**经查仍成立**：硬依赖恰为 Fabric API + FCAP；`build.gradle` 零个 `include(`，JiJ=0 | 只改命名那半，**别整条删** |
| 核心纪律 **第 3 条** | 「排除 = 隐形」的前提不存在：新基 `sourceSets.main.java.exclude` **0 条**。但「三查」里的 entrypoint / mixins.json 两查对**我们新增的类**依然有效（本轮实证：GameTest 不登记 entrypoint 就永不执行） | 改写成「凡我们新增的类必查 entrypoint 与 mixins.json 登记」 |
| 核心纪律 **第 4 条** | frozen-node / SCC / measurement-pass 是逐个解除排除时的依赖排序法，本分支无适用对象 | 标注「本分支不适用」，别删（做 26.2 时可能复用） |
| 核心纪律 **第 11 条** | 已自带「动到时复核」标注。其中「JEI 27 变体区分」要按**本分支的 JEI 29.22** 重新确认；「客户端不再持有 `RecipeManager`」几乎肯定仍成立——审计 §3.K 的祭坛配方同步正为它存在 | 不动，动到时逐条验 |

**不要一次性重写 `CLAUDE.md`**，那会把继承来的经验一起冲掉；逐条改并注明「原文为 1.21.11 语境，
本分支据 X 修正」——下一个人复用的是理由，不是结论。

⚠️ **这件事本身是一条教训**：把方法论整份复制过去，**继承的不只是经验，还有它的前提**。
凡复制方法论到新语境，必须逐条问「这条依赖的前提在新语境还成立吗」，而不是只改显眼的版本号。
建议改完后把这条追加进 `CLAUDE.md` 的证伪表。
