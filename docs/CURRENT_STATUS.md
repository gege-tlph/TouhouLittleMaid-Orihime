...
| **TACZ R2 可能已给出弹药源入口** | 我们钉 `26.1.2_R1`（8-12），用**四个 mixin** 打其内部类；而 `26.1.2_R2`（**8-16，比本兼容刀晚一天**）发布说明称 port 了 1.21.11 R2 的 *"compatibility hooks … and ammo queries"*。我们提的 issue（对方仓库 #46）已关闭且**无维护者回复**，故只能**读 R2 的 jar 定案**——发布说明不构成证据，要 `javap` 确认符号与签名。对上即可退役对应 mixin，把四处「对方一重构就断」的耦合减到零 |
