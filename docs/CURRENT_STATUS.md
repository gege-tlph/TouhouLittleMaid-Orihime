...
- ✅ 抱姿（F5 第三人称）、Shift 棋盘预览与提示行、残局道具右键载入、
  ...
  祭坛四配方、配置菜单两栏与保存生效（单人）、`chest/random_board_state` 的 `/loot`
...
| 待验 | 为什么这轮验不到 | 怎么验 |
|---|---|---|
| 三张背包战利品表的 `/loot` | 用户尚未跑（命令已给：`furnace_or_crafting_table_backpack` / `ender_chest_backpack` / `tank_backpack`，各跑 ~10 次，空权重约 80%；tank 出品 tooltip 须带岩浆 mB） | 单人档即可 |
| 饮用音效（女仆喂主人喝东西） | 条件难凑（女仆持药水/牛奶 + 主人扣血被喂），本轮未确认 | 单人档即可 |
