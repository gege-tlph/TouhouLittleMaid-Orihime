...
| **预期结果** | ① 雪球**在目标身上炸开**（原版命中即放 8 个雪球粒子并消失），不再落在目标脚前的地面上 ② 挨打后应战起得来，不被雪仗一瞬夺回 |
...
**也别看击退**：它到底有没有仍未定案——2026-08-19 已经有过一次真正命中，而用户报「仍旧没有击退伤害」，混淆变量（创造模式）尚未排除，见下。

**✅ ③ 的前一半 2026-08-19 实测通过：雪球能砸到人了**（用户实测，出手高度那刀生效）。
后一半（打雪仗期间挨打能否正常应战）**当轮未报，仍开放**。

⚠️ **同轮报出一个仍未定案的观察：命中了，却「没有击退、没有伤害」。** 已取证的部分：

| 事实 | 取证 |
|---|---|
| **0 伤害是原版设计**，不是缺陷 | `Snowball.onHitEntity` 字节码：烈焰人 3 点，**其余一律 `hurt(source, 0)`**。改它属超基准改上游行为，**不要动** |
| **击退段与伤害量无关** | `LivingEntity.hurtServer` 里击退那一段的闸门是「这是不是一次新命中」（局部量仅在 `invulnerableTime` 冷却内置 false），0 伤害照样走到 `knockback(0.4, dx, dz)` |
| **击退方向不看归属** | `Projectile.calculateHorizontalHurtKnockbackDirection` 方法体就是 `getDeltaMovement().x / .z`——所以「上游把归属去掉」不背这个锅 |
| **我们的子类不碰伤害路径** | 只覆写 `tick` 与 `canHitEntity` |
| **有一道闸门能完整解释「命中却什么都不发生」** | `Player.hurtServer` 首查 `isInvulnerableTo`、次查 `abilities.invulnerable`——**创造/旁观直接 return false**（无伤害、无击退、无受击闪烁），而粒子爆开走的是 `onHit → broadcastEntityEvent(3)`，与 hurt 无关，照常发生 |

**下一步只有一问**：那次实测玩家是不是创造模式。是 → 现表现全对，本项收尾；
否 → 上 `[TLM-QA-*]` 桩看 `hurtServer` 停在哪一道，**别再猜**（这条已经猜错过一轮）。
⚠️ 判据依赖「用户当时怎么操作的」，**先问，别假设**。
