...
| 应战期**弓/弩手感不如上游** | 用户裁定「还原这两个远程行为、枪械不还原」。取证：弓/弩的**工作任务与上游逐字相同**，差异全在 B1 那两条 AnyItem 变体丢掉的三个输入上（快速射击附魔 / `MAID_SHOOT_COOLDOWN` 属性 / 每把武器自己的射程）。三处按上游同式补回，枪走另一个类未动，B1 的站定与敌我判定按裁定保留（`94338b210`） |
